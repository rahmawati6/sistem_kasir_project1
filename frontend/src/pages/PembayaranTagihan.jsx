...
const emptyForm = { jenis_layanan: 'pln', nomor_pelanggan: '', nama_pelanggan: '', jumlah_tagihan: '' }
...
export default function PembayaranTagihan() {
  ...
  useEffect(() => { fetchData() }, [])
  const fetchData = async () => { try { const res = await api.get('/tagihan'); setData(res.data) } catch (e) {} }

  const handleSubmit = async (e) => {
    e.preventDefault(); setLoading(true)
    try {
      await api.post('/tagihan', { ...form, jumlah_tagihan: parseFloat(form.jumlah_tagihan) })
      ...
      {showForm && (
        <section className="brilink-form-card">
          ...
          <form onSubmit={handleSubmit} className="brilink-form-grid">
            ...
            <label className="field-group"><span>Nama Pelanggan</span><input type="text" value={form.nama_pelanggan} onChange={e => setForm({ ...form, nama_pelanggan: e.target.value })} required /></label>
            <label className="field-group"><span>Jumlah Tagihan</span><input type="number" value={form.jumlah_tagihan} onChange={e => setForm({ ...form, jumlah_tagihan: e.target.value })} required /></label>
      ...
      <section className="brilink-table-panel">
        ...
        <div className="brilink-table-wrap">
          <table className="brilink-table">
            <thead><tr><th>Kode</th><th>Tanggal</th><th>Layanan</th><th>Pelanggan</th><th>Tagihan</th><th>Admin</th><th>Total</th></tr></thead>
            <tbody>{data.map(t => (
              <tr key={t.id}>
